...
pOutlierFinderChiSqNoCorr=function(DataList, Fstbar, dfInferred, qthreshold=0.05, Hmin=0.1){
  ...
  if(any(!is.finite(pListRightTail)) || any(pListRightTail < 0 | pListRightTail > 1)) {
    stop(paste0(
      "OutFLANK cannot estimate q-values: non-finite or invalid p-values were " ,
      "generated for ", sum(!is.finite(pListRightTail)), " of ",
      length(pListRightTail), " eligible loci."), call. = FALSE)
  }

  qtemp=qvalue(pListRightTail,fdr.level=qthreshold,pi0.method="bootstrap")
  if(length(qtemp$qvalues) != length(pListRightTail) ||
     any(!is.finite(qtemp$qvalues))) {
    stop(paste0(
      "OutFLANK cannot estimate q-values: qvalue returned an invalid result for ",
      length(pListRightTail), " eligible loci."), call. = FALSE)
  }
