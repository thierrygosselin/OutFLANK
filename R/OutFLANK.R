...
pOutlierFinderChiSqNoCorr=function(DataList, Fstbar, dfInferred, qthreshold=0.05, Hmin=0.1){
  ...
  qtemp <- tryCatch(
    ...
    error = function(error) {
      stop(paste0(
        "OutFLANK cannot estimate q-values for ", length(pListRightTail),
        " eligible loci (FSTbar=", format(Fstbar), ", dfInferred=",
        format(dfInferred), "). qvalue reported: ", conditionMessage(error)),
        call. = FALSE)
    }
  )
